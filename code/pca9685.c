...
error_t pca9685_init(void **pca9685_info_loc, uint8_t pwm_freq)
{
    ...
    {
        dbg_prnt_err("Could not allocate memory for pca9685_info_t.");
        return ERR_NOMEM;
    }
    ...
    if (pca9685_info->fd < 0)
    {
        ...
        dbg_prnt_err("Failed to open the i2c bus connected to PCA9685. Check that the connection to the board is reliable.");
        perror("open");
        return ERR_CRIT;
    }
    ...
    {
        free(pca9685_info); /* Avoid memory leak. */
        dbg_prnt_err("Failed to reset the PCA9685 chip to default values hence register state is unknown.");
        return ERR_CRIT;
    }
    ...
    {
        free(pca9685_info); /* Avoid memory leak. */
        dbg_prnt_err("Failed to set the prescale value.");
        return ERR_CRIT;
    }
    ...
    {
        free(pca9685_info); /* Avoid memory leak. */
        dbg_prnt_err("Failed to wake up PCA9685 from SLEEP mode.");
...
error_t pca9685_reset(void *pca9685_info_ptr)
{
    ...
    {
        dbg_prnt_err("Failed to send the general call address for software reset.");
        perror("ioctl");
        return ERR_I2C_CTRL;
    }
...
error_t pca9685_reg_set(void *pca9685_info_ptr, uint8_t const reg_addr, uint8_t const data)
{
    ...
    {
        dbg_prnt_err("Failed to send the slave address.");
        perror("ioctl");
        return ERR_I2C_CTRL;
    }
    if (i2c_smbus_write_byte_data(pca9685_info->fd, reg_addr, data) != 0)
    {
        dbg_prnt_err("Failed to send register address and a new register value.");
...
error_t pca9685_reg_get(void *pca9685_info_ptr, uint8_t const reg_addr, uint8_t *data)
{
    ...
    if (data_tmp < 0)
    {
        dbg_prnt_err("Failed to send register address and read the register value.");
...
error_t pca9685_reg_print_all(void *pca9685_info_ptr)
{
    ...
    for (uint8_t ch_i = 0; ch_i < PCA9685_REG_CH_NUM; ch_i++)
    {
        ...
        {
            dbg_prnt_err("Failed to read from the ON and/or OFF channel registers.");
            return ERR_I2C_READ;
        }
    }
